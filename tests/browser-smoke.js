...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('#tool-hero').textContent(), /支持查询/);
    ...
    await page.locator('#brand-common .chip', { hasText: '约克' }).click();
    await page.locator('#wk-model').fill('YS-DAC-45');
    await page.locator('#wk-query').click();
    await page.waitForLoadState('networkidle');
    assert.match(page.url(), /tool-fluoro-result\.html\?type=water/, 'water query must enter the water result');
    assert.match(await page.locator('#rh-brand').textContent(), /约克 · 螺杆离心主机/, 'water result must show the chiller category');
    assert.match(await page.locator('#gw-list').textContent(), /W01G/, 'water result must recommend the W01G controller');
    assert.match(await page.locator('#gw-section-title').textContent(), /适配控制器/);
    await open(page, baseUrl, 'pages/tab-tools.html');
