(function (root) {
  ...
  var MENU_ITEMS = [
    ['capture', '检修抓码', ''],
    ['upgrade', '设备升级', 'device-e50-upgrade.html'],
    ['devices', '设备列表', 'device-e50-devices.html'],
    ['service', '技术&服务', 'device-e50-service.html'],
    ['contact', '联系我们', 'feedback.html']
  ];
  function renderMenu(active) {
    var host = document.getElementById('side-menu');
    if (!host) return;
    var nav = MENU_ITEMS.map(function (item) {
      return '<a class="side-nav-item' + (active === item[0] ? ' active' : '') + '" href="' + (item[2] ? link(item[2]) : '#') + '" data-menu="' + item[0] + '"><span>' + item[1] + '</span><b>›</b></a>';
    }).join('');
